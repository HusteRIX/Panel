# pages.py  -  HusteRIX Gateway v9.2
...
<title>ورود · HusteRIX Gateway</title>
...
<div class="wrap">
  <div class="card">
    <div class="brand">
      <div class="brand-img"><img src="https://yt3.googleusercontent.com/vA6bYj1V386YmibpWRNFJtsRRqwfY_U9wnb7gmW90eRVXyNB7gAfjj1XPs5UX0cdKdQprrI=s160-c-k-c0x00ffffff-no-rj" alt="HusteRIX"></div>
      <div><div class="brand-name">HusteRIX</div><div class="brand-sub">HusteRIX Gateway · v9.2</div></div>
    </div>
    <h1>ورود به پنل</h1>
    ...
    <div class="footer"> آیدی سازنده<a href="https://t.me/HusteRIX
    " target="_blank"><i class="ti ti-brand-telegram"></i>@HusteRIX</a></div>
  </div>
</div>
<script>
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>HusteRIX Gateway · HusteRIX</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
...
<div class="mob-top">
  <div class="ml">
    ...
    <span class="mob-title">HusteRIX Gateway</span>
  </div>
  <div class="mob-right">
...
<aside class="sidebar" id="sb">
  ...
  <div class="logo">
    ...
    <div><div class="logo-name">HusteRIX</div><div class="logo-sub">HusteRIX Gateway · v9.2</div></div>
  </div>
  <div class="nav-wrap">
  ...
  <div class="sb-foot">
    ...
    <a class="tg-btn" href="https://t.me/HusteRIX" target="_blank" rel="noopener"><i class="ti ti-brand-telegram"></i> @HusteRIX</a>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">codebox HusteRIX Gateway v9.2 · Railway · 2025</span>
    <a class="df-link" href="https://t.me/HusteRIX" target="_blank"><i class="ti ti-brand-telegram"></i> t.me/HusteRIX</a>
  </div>
</section>
...
<script>
let isDark=localStorage.getItem('HusteRIX -theme')!=='light';
...
function toggleTheme(){isDark=!isDark;localStorage.setItem('HusteRIX-theme',isDark?'dark':'light');applyTheme(isDark)}
applyTheme(isDark);
...
<title>HusteRIX Sub · codebox</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      ...
      <div><div class="brand-name">codebox</div><div class="brand-sub">HusteRIX Gateway · v9.2</div></div>
    </div>
    <div class="top-actions">
      <button class="icon-btn" id="theme-toggle" onclick="toggleTheme()" title="تغییر تم"><i class="ti ti-sun" id="theme-icon"></i></button>
      <a class="icon-btn" href="https://t.me/HusteRIX" target="_blank" title="کانال تلگرام"><i class="ti ti-brand-telegram"></i></a>
    </div>
  </div>
  <div id="root">
    <div class="empty-state"><i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i>در حال بارگذاری...</div>
  </div>
  <div class="footer">کانال رسمی: <a href="https://t.me/HusteRIX" target="_blank">@HusteRIX</a> · HusteRIX Gateway v9.2</div>
</div>
<script>
const UUID_KEY='{uuid_key}';
let savedPw='';

let isDark=localStorage.getItem('HusteRIX-pub-theme')!=='light';
function applyTheme(dark){{
  document.documentElement.setAttribute('data-theme',dark?'dark':'light');
  document.getElementById('theme-icon').className='ti '+(dark?'ti-sun':'ti-moon');
}}
function toggleTheme(){{isDark=!isDark;localStorage.setItem('HusteRIX-pub-theme',isDark?'dark':'light');applyTheme(isDark)}}
applyTheme(isDark);
...
function renderContent(d){{
  ...
  window._HusteRIXSubUrl  = subUrl;
  window._HusteRIXSubName = d.name;
  window._HusteRIXLinks   = d.links.map(l => ({{
    vless : l.vless_link,
    ...
    <div class="sub-info">
      ...
      <div class="sub-sub-box">
        ...
        <button class="btn btn-pur" style="padding:7px 12px;font-size:10.5px"
          onclick="navigator.clipboard.writeText(window._HusteRIXSubUrl).then(()=>toast('لینک ساب کپی شد ✓','ok'))">
          <i class="ti ti-copy"></i> کپی لینک ساب
        </button>
        <button class="btn btn-g" style="padding:7px 12px;font-size:10.5px"
          onclick="showQR(window._HusteRIXSubName + ' — کل گروه', window._HusteRIXSubUrl)">
            ...
            <div class="cfg-bottom">
              ...
              <div class="cfg-actions">
                <button class="btn btn-p"
                  onclick="navigator.clipboard.writeText(window._HusteRIXLinks[${{i}}].vless).then(()=>toast('لینک کپی شد ✓','ok'))">
                  <i class="ti ti-copy"></i> کپی لینک
                </button>
                <button class="btn btn-g"
                  onclick="showQR(window._HusteRIXLinks[${{i}}].label, window._HusteRIXLinks[${{i}}].vless)">
...
function copyAllConfigs(){{
  const links=window._HusteRIXLinks||[];
